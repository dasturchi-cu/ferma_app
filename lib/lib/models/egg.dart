...
class Egg extends HiveObject {
  ...
  int get todayProduction {
    ...
    return production
        .where((prod) => prod.date.year == today.year &&
  ...
  int get todayBroken {
    DateTime today = DateTime.now();
    return brokenEggs
        .where((broken) => broken.date.year == today.year &&
  ...
  int get todayLarge {
    DateTime today = DateTime.now();
    return largeEggs
        .where((large) => large.date.year == today.year &&
  ...
  int get currentStock {
    int totalProduction = production.fold(0, (sum, prod) => sum + prod.trayCount);
    int totalSales = sales.fold(0, (sum, sale) => sum + sale.trayCount);
    int totalBroken = brokenEggs.fold(0, (sum, broken) => sum + broken.trayCount);
  ...
  int productionLastNDays(int days) {
    ...
    return production
        .where((p) => p.date.isAfter(start.subtract(const Duration(seconds: 1))) &&
            p.date.isBefore(DateTime(now.year, now.month, now.day).add(const Duration(days: 1))))
  ...
  int salesLastNDays(int days) {
    ...
    return sales
        .where((s) => s.date.isAfter(start.subtract(const Duration(seconds: 1))) &&
            s.date.isBefore(DateTime(now.year, now.month, now.day).add(const Duration(days: 1))))
  ...
  int brokenLastNDays(int days) {
    ...
    final start = DateTime(now.year, now.month, now.day)
        .subtract(Duration(days: days - 1));
    return brokenEggs
        .where((b) => b.date.isAfter(start.subtract(const Duration(seconds: 1))) &&
            b.date.isBefore(DateTime(now.year, now.month, now.day).add(const Duration(days: 1))))
  ...
  int largeLastNDays(int days) {
    ...
    final start = DateTime(now.year, now.month, now.day)
        .subtract(Duration(days: days - 1));
    return largeEggs
        .where((l) => l.date.isAfter(start.subtract(const Duration(seconds: 1))) &&
            l.date.isBefore(DateTime(now.year, now.month, now.day).add(const Duration(days: 1))))
  ...
  Map<String, dynamic> get productionStats {
    ...
    daily.forEach((day, count) {
      if (count > mostCount) {
        mostDay = day; mostCount = count;
      }
      if (count < leastCount) {
        leastDay = day; leastCount = count;
      }
    });
  ...
  Map<String, dynamic> get brokenStats {
    ...
    DateTime? minProd = production.isNotEmpty
        ...
        : null;
    DateTime minBroken = brokenEggs.map((b) => b.date).reduce((a, b) => a.isBefore(b) ? a : b);
    DateTime startDate = (minProd == null || minBroken.isBefore(minProd)) ? minBroken : minProd;
  ...
  Map<String, dynamic> toJson() {
    return {
      'id': id,
      'production': production.map((prod) => prod.toJson()).toList(),
      'sales': sales.map((sale) => sale.toJson()).toList(),
      'brokenEggs': brokenEggs.map((broken) => broken.toJson()).toList(),
      'largeEggs': largeEggs.map((large) => large.toJson()).toList(),
  ...
  factory Egg.fromJson(Map<String, dynamic> json) {
    return Egg(
      ...
      production: (json['production'] as List<dynamic>?)
          ?.map((prod) => EggProduction.fromJson(prod as Map<String, dynamic>))
          .toList() ??
          [],
      sales: (json['sales'] as List<dynamic>?)
          ?.map((sale) => EggSale.fromJson(sale as Map<String, dynamic>))
          .toList() ??
          [],
      brokenEggs: (json['brokenEggs'] as List<dynamic>?)
          ?.map((broken) => BrokenEgg.fromJson(broken as Map<String, dynamic>))
          .toList() ??
          [],
      largeEggs: (json['largeEggs'] as List<dynamic>?)
          ?.map((large) => LargeEgg.fromJson(large as Map<String, dynamic>))
          .toList() ??
          [],
